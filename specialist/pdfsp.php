...
<?php
    $_SESSION['date'] = date("d-m-Y", strtotime("+ 8 HOURS"));
    ...
    <div id="print">
        <div style = "margin:10px;">    <br />      
            ...
            <label>Date:
                ...
                <?php echo  $_GET['id'];
                //get name
                 $q1 = $conn->query("SELECT concat(firstname,' ',middlename,' ',lastname) as name FROM `patient` WHERE pat_no = '$_GET[id]'") or die(mysqli_error());
                        $f1 = $q1->fetch_array();
                    ?>
                    <label>Name:
                <?php echo $f1['name'];
            ...
            <center>
                ...
                <?php
//get total
                $q = $conn->query("SELECT COUNT(*) as total FROM `patient_hrecold` WHERE pat_no = '$_GET[id]'") or die(mysqli_error());
                        $f = $q->fetch_array();

                    $query = $conn->query("select date,patient_hrecold.pat_no,concat(patient.firstname,' ',patient.lastname) as name,remark,complaints,specialist_remarks,lab_remarks,prescription,concat(doctor.firstname,' ',doctor.lastname) as doctor,patient_hrecold.hospital from patient_hrecold LEFT JOIN patient on patient.pat_no=patient_hrecold.pat_no left join doctor on doctor.user_id=patient_hrecold.doctor WHERE patient_hrecold.pat_no = '$_GET[id]' order by date desc ") or die(mysqli_error());
