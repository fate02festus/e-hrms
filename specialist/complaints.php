...
<body>
	...
	<div>
		...
		<div style = "display:none;" id = "com" class = "panel panel-success">	
			...
			<?php
			//$query = $conn->query("SELECT * FROM `hrecold_temp` WHERE `pat_no` = '$_GET[id]' order by date desc") or die(mysqli_error());
			...
			?>
				<form method = "POST" action="add_complaints.php?id=<?php echo $f['pat_no']?>&lastname=<?php echo $f['lastname']?>" enctype = "multipart/form-data">
					...
					<div class = "form-group">
						...
						<input class = "form-control" textarea style = "resize:3" name = "remarks"  value = "<?php echo $tt['remark']?>" type = "text" readonly ="readonly"></textarea>
						<br />
						<label>Specialist Remarks:</label>
						<textarea style = "resize:3;" name = "specs" class = "form-control" type = "text" required="required"></textarea>	
						<label>Refer to Lab</label><input type="checkbox" name="specialist" value="1">
		...
		<button class = "btn btn-primary" id = "show_com"><i class = "glyphicon glyphicon-plus">ADD MEDICATION</i></button>
		<a href = "pdfsp.php?id=<?php echo 	$pat ?>" class = "btn btn-sm btn-print"><span class = "glyphicon glyphicon-pencil"></span> Print</a>
		<div class = "panel-body">
			<?php
				...
				$q = $conn->query("SELECT *,patient_hrecold.hospital as hos,concat(firstname,' ',lastname) as doctor FROM `patient_hrecold` LEFT join doctor on doctor.user_id=patient_hrecold.doctor  WHERE `pat_no` = '$_SESSION[pat_no]' ORDER BY `date` DESC") or die(mysqli_error());	
					while($f = $q->fetch_array()){
						echo "<label style = 'color:#3399f3;'>Doctor: ".$f['doctor']."</label>"."Remarks: "."<textarea  style = 'resize:none;' disabled = 'disabled' class = 'form-control'>".$f['remark']."</textarea>"."Specialist Remarks: "."<textarea  style = 'resize:none;' disabled = 'disabled' class = 'form-control'>".$f['specialist_remarks']."</textarea>"."Lab Remarks: "."<textarea  style = 'resize:none;' disabled = 'disabled' class = 'form-control'>".$f['lab_remarks']."</textarea>"."Hospital: ". "<label style = 'color:#3399f3;'>".$f['hos']."</label>"."  Date:".$f['date']."<br /><hr style = 'border:1px solid #eee;' />";
					}
				?>
		</div>
	</div>
