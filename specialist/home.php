	...
	if(ISSET($_POST['search_patient'])){
         $date = date('d/m/Y');
			...
			if($pno!=null && $huno==null){
				$q1 = $conn->query("SELECT * FROM `patient_hrecold` WHERE `pat_no` = '$pno' and `date`='$date' and `specialist_type`='$_SESSION[specs]'") or die(mysqli_error());
			$c1 = $q1->num_rows;
			if($c1 > 0){
				 $f = $q1->fetch_array();
				...
				echo " <script>alert('Patient does not exist. Kindly check...')</script>";
				//echo " <script>setTimeout(\"location.href='patient.php';\",150);</script>";
			}
			}else if($huno!=null && $pno==null ){
				$q1 = $conn->query("SELECT * FROM `patient_hrecold` WHERE `phone` = '$huno' and `date`='$date' and `specialist_type`='$_SESSION[specs]'") or die(mysqli_error());
				$c1 = $q1->num_rows;
				if($c1 > 0){
					 $f = $q1->fetch_array();
					$_SESSION['pat_no']=$f['pat_no'];
