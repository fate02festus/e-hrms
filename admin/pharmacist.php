	...
	<div id = "content">
		...
		<div id = "add" class = "panel panel-success">	
			<div class = "panel-heading">
				<label>ADD PHARMACIST ACCOUNT</label>
				...
				<form id = "form_user" method = "POST" enctype = "multi-part/form-data">
					...
					<div class = "panel-body">
						<div class = "form-group">
							<label for = "username">Pharmacist Code: </label>
							<input class = "form-control" name = "unique_code" type = "text" value = "<?php echo $itr_no ?>"  required = "required" disabled = 'disabled'>
						</div>
						<div class = "form-group">
							<label for = "username">Username: </label>
							<input class = "form-control" name = "username" type = "email" placeholder="Enter Username" required = "required">
						</div>
						<div class = "form-group">
						...
						<div class = "form-group">
							...
							<input class = "form-control" type = "text" name = "lastname" required = "required" required = "required" pattern="[A-Za-z]{1,15}" title="Enter characters only">
						</div>
						<div class = "form-group">
							<label for = "lastname">Phone: </label>
							<input class = "form-control" type = "number" min="1" name = "contacts" required = "required" pattern="[0-9]{9,12}" title="Enter characters value of length between 7 and 8 characters">
						</div>
						<div class = "form-group">
							<label for = "lastname">ID No.: </label>
							<input class = "form-control" type = "number" min="1"  name = "idno" required ="required" pattern="[0-9]{7,8}" title="Enter characters value of length between 9 and 12 characters">
						</div>
						<div class = "form-group">
						...
						<div class = "form-group">	
							...
							<input class = "form-control" name = "confpassword" minlength = "3" type = "password" required = "required">
						</div>
						
							<button class = "btn btn-primary" name = "save_user" ><span class = "glyphicon glyphicon-save"></span> SAVE</button>
							<br />
					</div>	
					<?php require 'add_pharmacist.php'?>
					</div>
				</form>			
			</div>	
		</div>	
		<div class = "panel panel-primary">
			<div class = "panel-heading">
				<label>ACCOUNTS / PHARMACIST</Label>
			</div>
			<div class = "panel-body">
				...
				<table id = "table" class = "display" width = "100%" cellspacing = "0">
					...
					<tbody>
					<?php
						$query = $conn->query("SELECT * FROM `pharmacist` ORDER BY `user_id` DESC") or die(mysqli_error());
					...
					?>
						<tr>
							...
							<td><center><a href = "edit_pharmacist.php?id=<?php echo $fetch['user_id']?>&lastname=<?php echo $fetch['lastname']?>"class = "btn btn-sm btn-warning"><i class = "glyphicon glyphicon-edit"></i> Update</a> <a onclick = "delete_user(this); return false;"  href = "delete_pharmacist.php?id=<?php echo $fetch['user_id']?>&lastname=<?php echo $fetch['lastname']?>" class = "btn btn-sm btn-danger"><i class = "glyphicon glyphicon-remove"></i> Archive</a></center></td>
						</tr>
					<?php
						}
						$conn->close();
